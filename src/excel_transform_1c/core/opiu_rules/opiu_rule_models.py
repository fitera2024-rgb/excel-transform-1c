...
class ERPSourceRule:
    ...
    code: str
    formula_condition: str
    # Exact article-dimension scopes parsed from the ERP source condition.
    # ``disclosure_groups`` contains only С1/КС1 hierarchy roots; values from
    # other dimensions (for example С2 sales channels) are never promoted to
    # disclosure groups.
    disclosure_groups: tuple[str, ...] = ()
    article_values: tuple[str, ...] = ()
